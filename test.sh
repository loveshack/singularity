...
stest 0 sudo "$TEMPDIR/bin/singularity" create -s 568 "$CONTAINER"
stest 0 sudo "$TEMPDIR/bin/singularity" bootstrap "$CONTAINER" "$STARTDIR/examples/centos.def"
...
stest 0 sudo "$TEMPDIR/bin/singularity" copy "$CONTAINER" -a singularity /
stest 1 singularity run "$CONTAINER" true
stest 0 sudo "$TEMPDIR/bin/singularity" exec -w "$CONTAINER" chmod 0755 /singularity
...
stest 0 sudo "$TEMPDIR/bin/singularity" shell -w "$CONTAINER" -c true
stest 0 sudo "$TEMPDIR/bin/singularity" exec -w "$CONTAINER" true
stest 0 sudo "$TEMPDIR/bin/singularity" run -w "$CONTAINER" true
...
stest 1 sudo "$TEMPDIR/bin/singularity" exec "$CONTAINER" touch /writetest.fail
stest 0 sudo "$TEMPDIR/bin/singularity" exec -w "$CONTAINER" touch /writetest.pass


/bin/echo
/bin/echo "Checking Bootstrap on existing container..."
stest 0 sudo "$TEMPDIR/bin/singularity" bootstrap "$CONTAINER"
stest 0 singularity exec "$CONTAINER" test -f /environment
stest 0 sudo "$TEMPDIR/bin/singularity" exec -w "$CONTAINER" rm /environment
...
stest 0 sudo "$TEMPDIR/bin/singularity" export -f out.tar "$CONTAINER"
stest 0 mkdir out
stest 0 sudo tar -C out -xvf out.tar
stest 0 sudo chmod 0644 out.tar
stest 0 sudo rm -f "$CONTAINER"
stest 0 sudo "$TEMPDIR/bin/singularity" create -s 568 "$CONTAINER"
stest 0 sh -c "cat out.tar | sudo "$TEMPDIR/bin/singularity" import $CONTAINER"
